// Cove Compress — the video compressor, packaged as a Tool. It owns its own markup (rendered
// into ctx.host) and contributes its commands to the shell's palette. Compression itself is
// delegated to the engine orchestrator (compressor.ts), which auto-selects WebCodecs or
...
export function mount(ctx: ToolContext): () => void {
  ...
  const probeMeta = (file: File): Promise<{ duration: number; width: number; height: number }> =>
    new Promise((resolve) => {
  ...
  // Global drag guards keep a stray drop from navigating the page away; torn down on cleanup.
